...
import {setReservation, toReservationCreateReqDtoMapper} from "../redux/features/reservationSlice";
...
const PaymentForm = ({reservationDetails, roomSelectedList}) => {
  ...
  const [createReservation, {isLoading: isCreateReservationLoading}] = useCreateReservationMutation()
  const [chargePayment, {isLoading: isChargePaymentLoading}] = useChargePaymentMutation()

  const chargePaymentOnClick = async (token) => {
    try {
      let response = await createReservation(toReservationCreateReqDtoMapper({
  ...
  return isCreateReservationLoading || isChargePaymentLoading ? (
    ...
    <div key={`payment-form`}>
      ...
      <StripeCheckout
        ...
        currency={"USD"}
        name={"Diamond hotel"}
        description={"Reservation (" + reservationDetails.checkIn.toISOString().split("T")?.at(0) + "/" + reservationDetails.checkOut.toISOString().split("T")?.at(0) + ")"}
      >
        <button
