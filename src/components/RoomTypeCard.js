...
const RoomTypeCard = ({ id, availableRooms, selectedRooms, updateRoomTypeDetails }) => {
  ...
  const [getRoomTypeEquipment, { isLoading: isLoadingRoomTypeEquipment }] =
    useGetRoomTypeEquipmentMutation();
  ...
  return isLoadingRoomType || isLoadingRoomTypeEquipment ? (
    ...
    <div
      ...
      <div className="cursor-pointer absolute right-[0] top-[0] mr-5 mt-2 ">
        <RoomTypeDescription
          id={roomType?.id}
          pricePerHotelNight={roomType?.pricePerHotelNight}
          name={roomType?.name}
