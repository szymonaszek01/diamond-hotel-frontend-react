...
const SelectedWeatherItem = ({weather, index}) => {
  ...
  return (
    <div key={`weather-${index}`} className={"flex flex-col w-full gap-5 text-center sm:text-start"}>
      <p className={`flex flex-col text-sm text-dimWhite font-poppins font-thin leading-10 sm:leading-8 gap-5`}>
        <span className={"text-3xl font-semibold text-white leading-[50px] sm:leading-8"}>Upcoming Weather Forecast</span>
        <span>
            Stay informed about the weather conditions in your area and plan ahead with our upcoming weather forecast..
        </span>
      </p>
      <motion.div key={`weather-${index}`} initial={{opacity: 0, scale: 0.5}} animate={{opacity: 1, scale: 1}}
        ...
        <div className="flex flex-col items-center text-center gap-4 xs:gap-0 xs:text-start lg:items-start">
          ...
          <div className="flex flex-col xs:flex-row items-center gap-0 sm:gap-5">
            <div className="flex flex-col">
              <h2
                className="font-poppins text-white text-4xl xs:text-6xl font-thin">{getMeasurementByName(measurement.temperatureDay)?.value.toFixed(0)}{getMeasurementByName(measurement.temperatureDay)?.unit}</h2>
        ...
        <div className="flex flex-col items-center lg:items-start">
          <p
            className="font-poppins font-bold text-white text-center xs:text-start text-sm">{weather?.description?.charAt(0).toUpperCase() + weather?.description?.slice(1)}</p>
...
const DailyWeatherItem = ({weather, index}) => {
  ...
  return (
    <div
      className={`border-white border-[1px] lg:border-none relative flex flex-col xs:flex-row lg:flex-col gap-2 rounded-[10px] p-4 items-center justify-center cursor-pointer ${index === 0 ? "bg-black-gradient" : "bg-[#00000018]"}`}>
      <p
        className="font-poppins font-bold text-white text-xs break-all xs:break-normal">{new Date(weather?.measuredAt ?? Date.now()).toLocaleDateString()}</p>
      <img src={weather?.icon} alt="close" className="w-[50px] h-auto"/>
      <p
        className="font-poppins font-thin text-white text-xs break-all xs:break-normal">{getMeasurementByName(measurement.temperatureDay)?.value.toFixed(0)} {getMeasurementByName(measurement.temperatureDay)?.unit}</p>
      <p
        className="font-poppins font-thin text-white text-xs break-all xs:break-normal">{getMeasurementByName(measurement.pressure)?.value.toFixed(0)} {getMeasurementByName(measurement.pressure)?.unit}</p>
      <p
        className="font-poppins font-thin text-white text-xs break-all xs:break-normal">{getMeasurementByName(measurement.wind)?.value.toFixed(2)} {getMeasurementByName(measurement.wind)?.unit}</p>
    </div>
  )
}
