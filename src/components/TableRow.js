...
import {TableCell, TableActionMenu} from "./index";
...
const TableRow = ({index, columnList, cellList, isLastRow, actionList}) => {
  ...
  return (
    <div key={`row-${randomCode(7)}`}
         className={`${isEven ? 'border-y-[1px] border-[#ebeef1]' : ''} w-full flex flex-col sm:flex-row gap-2 sm:gap-0 justify-center sm:justify-between items-center p-4 ${isLastRow ? 'border-b-[1px] border-[#ebeef1]' : ''}`}>
      {
